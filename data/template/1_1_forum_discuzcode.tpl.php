...
$return .= <<<EOF
Для просмотра скрытого контента необходимо иметь на балансе: {$creditsrequire} монет. Ваш баланс: {$_G['member']['credits']} монет.</div>
EOF;
?><?php 
return $return;
}
...
?><?php
$return = <<<EOF
<div class="locked"> Вам доступен скрытый контент, так как ваш баланс: {$creditsrequire} монет.</div>
{$message}<br /><br />
...
?><?php
$return = <<<EOF
<div class="blockcode"><div id="{$randomid}"><ol><li>{$code}</ol></div><em onclick="copycode($('{$randomid}'));">Скопировать код</em></div>
EOF;
?><?php 
return $return;
}
...
?><?php
$return = <<<EOF
<div class="showhide"><h4>Скрытый контент</h4>\\1</div>
...
$return .= <<<EOF
Скрытоый контент будет доступен после отправки сообщения. <a href="forum.php?mod=post&amp;action=reply&amp;fid={$_G['fid']}&amp;tid={$_G['tid']}" onclick="showWindow('reply', this.href)">Отправить сообщение</a>.</center></div>
EOF;
?><?php 
return $return;
}
...
<div class="tip_c">
<p class="y">{$attach['dateline']} Загрузить</p>
<p>Кликните по файлу для скачивания</p>

EOF;
 if(!$attach['attachimg'] && $_G['getattachcredits']) { 
$return .= <<<EOF
Вам за скачивание файла: {$_G['getattachcredits']}<br />
EOF;
 } 
$return .= <<<EOF
...
EOF;
 if($attach['readperm']) { 
$return .= <<<EOF
, Доступ: <strong>{$attach['readperm']}</strong>
EOF;
 } 
$return .= <<<EOF
, Скачиваний: {$attach['downloads']}
EOF;
 if(!$attach['attachimg'] && $_G['getattachcredits']) { 
$return .= <<<EOF
, Вам за скачивание файла: {$_G['getattachcredits']}
EOF;
 } 
$return .= <<<EOF
</p>
...
Стоимость: <strong>{$attach['price']} {$_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['unit']}{$_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['title']}</strong> &nbsp;[<a href="forum.php?mod=misc&amp;action=viewattachpayments&amp;aid={$attach['aid']}" onclick="showWindow('attachpay', this.href)" target="_blank">Отчет</a>]

EOF;
 if(!$attach['payed']) { 
$return .= <<<EOF

&nbsp;[<a href="forum.php?mod=misc&amp;action=attachpay&amp;aid={$attach['aid']}&amp;tid={$attach['tid']}" onclick="showWindow('attachpay', this.href)">Купить</a>]
...
<em class="xg1">({$attach['attachsize']}, Скачиваний: {$attach['downloads']})</em>
</p>
<div class="tip tip_4" id="aid{$attach['aid']}_menu" style="display: none" disautofocus="true">
<div>
<p>
<a href="forum.php?mod=attachment{$is_archive}&amp;aid={$aidencode}&amp;nothumb=yes" target="_blank">Скачать файл</a>

EOF;
 if(helper_access::check_module('album')) { 
$return .= <<<EOF

&nbsp;<a href="javascript:;" onclick="showWindow(this.id, this.getAttribute('url'), 'get', 0);" id="savephoto_{$attach['aid']}" url="home.php?mod=spacecp&amp;ac=album&amp;op=saveforumphoto&amp;aid={$attach['aid']}&amp;handlekey=savephoto_{$attach['aid']}">Сохранить в альбоме</a>

EOF;
 } if($firstpost && $_G['fid'] && $_G['forum']['picstyle'] && ($_G['forum']['ismoderator'] || $_G['uid'] == $attach['uid'])) { 
$return .= <<<EOF

&nbsp;<a href="forum.php?mod=ajax&amp;action=setthreadcover&amp;aid={$attach['aid']}&amp;fid={$_G['fid']}" onclick="showWindow('setcover{$attach['aid']}', this.href)">Установить обложки</a>
...
<p>
<span class="y">{$attach['dateline']} Загрузить</span>
...
&nbsp;[<a href="forum.php?mod=misc&amp;action=attachpay&amp;aid={$attach['aid']}&amp;tid={$attach['tid']}" onclick="showWindow('attachpay', this.href)" target="_blank">Купить</a>]
...
<br>
<a href="member.php?mod=logging&amp;action=login" onclick="showWindow('login', this.href+'&referer='+encodeURIComponent(location));">Войдите/Зарегистрируйтесь, чтобы увеличить</a>
</div>
...
<div class="xs0">
<p><strong>{$attach['filename']}</strong> <em class="xg1">({$attach['attachsize']}, Скачиваний: {$attach['downloads']})</em></p>
<p>
<a href="forum.php?mod=attachment{$is_archive}&amp;aid={$aidencode}&amp;nothumb=yes" target="_blank">Скачать файл</a>
...
<p class="xg1 y">{$attach['dateline']} Загрузить</p>
...
<em class="xg1">({$attach['attachsize']}, Скачиваний: {$attach['downloads']}
EOF;
 if($attach['price']) { 
$return .= <<<EOF
, Стоимость: {$attach['price']} {$_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['unit']}{$_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['title']}
EOF;
 } 
$return .= <<<EOF
)</em>
...
<div class="tip_c xs0">
<div class="y">{$attach['dateline']} Загрузить</div>
...
$return .= <<<EOF
<br />Доступ: {$attach['readperm']}
EOF;
 } } if(!IS_ROBOT && !$guestviewthumb) { if($attach['price']) { 
$return .= <<<EOF

<br />Стоимость: {$attach['price']} {$_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['unit']}{$_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['title']}	&nbsp;<a href="forum.php?mod=misc&amp;action=viewattachpayments&amp;aid={$attach['aid']}" onclick="showWindow('attachpay', this.href)" target="_blank">[Отчет]</a>
...
$return .= <<<EOF
<br />Вам за скачивание файла: {$_G['getattachcredits']}
EOF;
 } 
$return .= <<<EOF
