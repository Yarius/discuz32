<?php
/**
 * Created by 0 Ring Inc.
 * Project: Dizcus X3 Yandex RSS module
 * Date: 15.10.14
 * Time: 5:40
 */
...
echo 	"<?xml version=\"1.0\" encoding=\"".$charset."\"?>\n".
    ...
    "    <generator>{$_G[siteurl]}</generator>\n".
    "    <lastBuildDate>".gmdate('r', TIMESTAMP)."</lastBuildDate>\n".
    "    <ttl>$ttl</ttl>\n".
    "    <image>\n".
    "      <url>{$_G[siteurl]}for_yandex_logo.png</url>\n". // путь к логотипу для яндекса
    "      <title>{$_G[setting][bbname]}</title>\n".
    ...
    if($alldata) {
        foreach($alldata as $article) {
            if(TIMESTAMP - $article['lastupdate'] > $ttl * 60) {
                ...
                $querySummary = DB::query("SELECT * FROM ".DB::table('portal_article_title')." 	WHERE `aid` = '".$article['aid']."'	LIMIT 1");
                while ($value = DB::fetch($querySummary)) {
                    $feed_list = $value['summary'];
                }
                echo 	"    <item>\n".
                    ...
                    "      <yandex:full-text>".dhtmlspecialchars($feed_list)."</yandex:full-text>\n".
                    ($attachfile ? '<enclosure url="'.$filename.'" length="'.$attachsize.'" type="image/jpeg" ></enclosure>'."\n" : '').
