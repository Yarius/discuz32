...
function extrafunc_atListMenu(tag, op) {
	...
	if(!op) {
		...
		return 'Введите имя пользователя: <br /><input type="text" id="atkeyword" style="width:240px" value="" class="px" onkeydown="atEnter(event, \'atListSet\')" onkeyup="atFilter(this.value, \'at_list\',\'atListSet\',event, true);" /><div class="p_pop" id="at_list" style="width:250px;"><ul><li>@Аккаунт друга для напоминания</li></ul></div>';
	} else {
		if($('atkeyword').value) {
...
function atMenu(x, y) {
	...
	$('at_menu').innerHTML = '<img src="' + IMGDIR + '/loading.gif" class="vm"> Загрузка... ';
}
...
function atFilter(kw, id, call, e, nae) {
	...
	if(nae || !atEnter(e, call)) {
		...
		if(atResult.length) {
			...
			$(id).innerHTML = '<ul>' + newlist + '<li class="xg1">@Аккаунт друга для привлечения внимания</li></ul>';
		} else {
