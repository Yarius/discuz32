...
(function(){
	...
	Drag.prototype = {
		...
		checkEdit : function (ele) {
			...
			if (!$(id+'_edit')) {
				...
				dom.id = id+'_edit';
				dom.innerHTML = '<span id="'+id+'_edit_menu">Изменить</span>';
				ele.appendChild(dom);
		...
		setClose : function () {
			if (!this.isChange) {
				window.onbeforeunload = function() {
					return 'Ваши данные были изменены, но не были сохранены. Если желаете сохранить данные, нажмите: Отмена и затем, сохраните данные.';
				};
			}
			this.isChange = true;
		},
	...
	DIY.prototype = {
		...
		cancel : function () {
			var flag = false;
			if (this.isChange) {
				flag = confirm(this.cancelConfirm ? this.cancelConfirm : 'Не забудьте сохранить настройки перед выходом! Вы действительно хотите выйти?');
			}
