...
function initEditor() {
	...
	for(var i = 0; i < buttons.length; i++) {
		if(buttons[i].id.indexOf(editorid + '_') != -1) {
			...
			if(buttons[i].id.substr(buttons[i].id.indexOf('_') + 1) == 'fullswitcher') {
				buttons[i].innerHTML = !editorisfull ? 'Экран+' : 'Экран-';
				buttons[i].onmouseover = function(e) {setEditorTip(editorisfull ? 'Свернуть' : 'Раскрыть');};
				buttons[i].onclick = function(e) {editorfull();doane();}
			} else if(buttons[i].id.substr(buttons[i].id.indexOf('_') + 1) == 'simple') {
				buttons[i].innerHTML = !simplodemode ? 'Панель-' : 'Панель+';
	...
	if($(editorid + '_fullswitcher') && BROWSER.ie && BROWSER.ie < 7) {
		$(editorid + '_fullswitcher').onclick = function () {
			showDialog('Ваш браузер не поддерживает эту функцию, обновите версию браузера!', 'notice', 'Совет');
		};
...
function savedataTime() {
	if(!autosave) {
		$(editorid + '_svdsecond').innerHTML = '<a title="Нажмите для автосохранения" href="javascript:;" onclick="setAutosave()">Автосохранение</a> ';
		return;
	}
	if(!savedatac) {
		savedatac = savedataInterval;
		...
		h = h < 10 ? '0' + h : h;
		m = m < 10 ? '0' + m : m;
		setEditorTip('Данные сохранены ' + h + ':' + m + ' ');
	}
	$(editorid + '_svdsecond').innerHTML = '<a title="Нажмите, чтобы отключить автосохранение" href="javascript:;" onclick="setAutosave()">' + savedatac + ' cек до сохранения</a> ';
	savedatac -= 10;
}

function setAutosave() {
	autosave = !autosave;
	setEditorTip(autosave ? 'Автосохранение включено' : 'Автосохранение отключено');
...
function editorfull(op) {
	...
	$(editorid + '_fullswitcher').innerHTML = editorisfull ? 'Экран-' : 'Экран+';
	initesbar();
}
...
function editorsimple() {
	if($(editorid + '_body').className == 'edt') {
		v = 'none';
		$(editorid + '_simple').innerHTML = 'Панель+';
...
function checklength(theform) {
	...
	showDialog('Текущая длина: ' + mb_strlen(message) + ' байт,' + (postmaxchars != 0 ? 'Лимит: ' + postminchars + ' до ' + postmaxchars + ' байт' : ''), 'notice', 'Проверить длину');
}
...
function discuzcode(cmd, arg) {
	...
	if(in_array(cmd, ['sml', 'inserthorizontalrule', 'url', 'quote', 'code', 'free', 'hide', 'aud', 'vid', 'fls', 'beginning', 'attach', 'image', 'pasteword', 'index', 'postbg', 'password']) || typeof EXTRAFUNC['showEditorMenu'][cmd] != 'undefined' || cmd == 'tbl' || in_array(cmd, ['fontname', 'fontsize', 'forecolor', 'backcolor']) && !arg) {
		...
		if(txt = getSel()) {
			...
			while(listvalue = prompt('Введите элемент списка\r\nОставьте пустым или нажмите Отмена, чтобы завершить', '')) {
				if(BROWSER.opera > 8) {
					listvalue = '\n' + '[*]' + listvalue;
		...
		loadData();
		setEditorTip('Данные были восстановлены');
	} else if(cmd == 'svd') {
		saveData();
		setEditorTip('Данные были сохранены');
	} else if(cmd == 'chck') {
		...
		showDialog('<div id="remotedowninfo"><p class="mbn">Загрузка удаленного изображения, пожалуйста, подождите ……</p><p><img src="' + STATICURL + 'image/common/uploading.gif" alt="" /></p></div>', 'notice', '', null, 1);
...
function showEditorMenu(tag, params) {
	...
	if(menu) {
		...
		switch(tag) {
			case 'url':
				str = 'Введите URL ссылки:<br /><input type="text" id="' + ctrlid + '_param_1" style="width: 98%" value="" class="px" />'+
					(selection ? '' : '<br />Введите текст ссылки:<br /><input type="text" id="' + ctrlid + '_param_2" style="width: 98%" value="" class="px" />');
				break;
			case 'forecolor':
			...
			case 'postbg':
				...
				break;
			case 'password':
				str = '<p class="pbn">Введите пароль <input type="text" id="' + ctrlid + '_param_1" size="10" value="" class="px" /></p>';
				break;
			case 'code':
				if(wysiwyg) {
			...
			case 'free':
				...
				var lang = {'quote' : 'Пожалуйста, введите цитату', 'code' : 'Пожалуйста, вставьте код', 'hide' : 'Пожалуйста, введите скрытый контент', 'free' : 'Введите текст, который будет виден перед скрытым'};
				str += lang[tag] + ':<br /><textarea id="' + ctrlid + '_param_1" style="width: 98%" cols="50" rows="5" class="txtarea"></textarea>' +
					(tag == 'hide' ? '<br /><label><input type="radio" name="' + ctrlid + '_radio" id="' + ctrlid + '_radio_1" class="pc" checked="checked" />Скрытый контет будет доступен после публикации сообщения</label><br /><label><input type="radio" name="' + ctrlid + '_radio" id="' + ctrlid + '_radio_2" class="pc" />Стоимость просмотра: </label> <input type="text" size="3" id="' + ctrlid + '_param_2" class="px pxs" /> денег<br /><br /><label>Срок действия:</label> <input type="text" size="3" id="' + ctrlid + '_param_3" class="px pxs" /> <br />Срок действия скрытого контента, в днях' : '');
				break;
			case 'tbl':
				str = '<p class="pbn">Строки: <input type="text" id="' + ctrlid + '_param_1" size="2" value="2" class="px" /> &nbsp; Столбцы: <input type="text" id="' + ctrlid + '_param_2" size="2" value="2" class="px" /></p><p class="pbn">Ширина: <input type="text" id="' + ctrlid + '_param_3" size="2" value="" class="px" /> &nbsp; Цвет фона: <input type="text" id="' + ctrlid + '_param_4" size="2" class="px" onclick="showColorBox(this.id, 2)" /></p><p class="xg2 pbn" style="cursor:pointer" onclick="showDialog($(\'tbltips_msg\').innerHTML, \'notice\', \'Помощь\', null, 0)"><img id="tbltips" title="Помощь" class="vm" src="' + IMGDIR + '/info_small.gif"> Помощь</p>';
				str += '<div id="tbltips_msg" style="display: none">“[tr=цвет]” Определить цвет строки<br />“[td=ширина]” Определить ширину колонки<br />“[td=расстояние,диапазон,ширина]” Определите параметры<br /><br />Быстрая письменная форма. Пример: <div class=\'xs0\' style=\'margin:0 5px\'>[table]<br />Название:|Discuz!<br />Версия:|X2.5<br />[/table]</div>Для разделения каждого столбца в таблице можете использовать символ "|". Вместо символа "|" можете использовать значение "\\|" и значение "\\n", для перевода строки.</div>';
				break;
			case 'aud':
				str = '<p class="pbn">Пожалуйста, введите адрес аудио файла:</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_1" class="px" value="" style="width: 220px;" /></p><p class="xg2 pbn">Поддерживаются форматы: wma mp3 ra rm и другие музыкальные форматы<br />Пример: http://server/audio.wma</p>';
				break;
			case 'vid':
				str = '<p class="pbn">Пожалуйста, введите адрес видео файла:</p><p class="pbn"><input type="text" value="" id="' + ctrlid + '_param_1" style="width: 220px;" class="px" /></p><p class="pbn">Ширина: <input id="' + ctrlid + '_param_2" size="5" value="500" class="px" /> &nbsp; Высота: <input id="' + ctrlid + '_param_3" size="5" value="375" class="px" /></p><p class="xg2 pbn">Поддержка вещания видео станций с веб-сайтов<br />Поддержка форматов: wmv, avi, rmvb, mov, swf, flv  и др.<br />Пример: http://server/movie.wmv</p>';
				break;
			case 'fls':
				str = '<p class="pbn">Пожалуйста, введите адрес Flash файла:</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_1" class="px" value="" style="width: 220px;" /></p><p class="pbn">Ширина: <input id="' + ctrlid + '_param_2" size="5" value="" class="px" /> &nbsp; Высота: <input id="' + ctrlid + '_param_3" size="5" value="" class="px" /></p><p class="xg2 pbn">Поддержка форматов: swf, flv и Flash роликов с сайта<br />Пример: http://server/flash.swf</p>';
				break;
			case 'beginning':
				str = '<p class="pbn">Введите адреc медиа-файла: </p><p class="pbn"><input type="text" id="' + ctrlid + '_param_1" class="px" value="" style="width: 220px;" /></p>';
				str += '<p class="pbn">Ссылка:</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_2" class="px" value="" style="width: 220px;" /></p>';
				str += '<p class="pbn">Шир.: <input id="' + ctrlid + '_param_3" size="5" value="" class="px" /> &nbsp; Выс.: <input id="' + ctrlid + '_param_4" size="5" value="" class="px" /></p>';
				str += '<p class="pbn">Воспроизвести с (сек.): <input id="' + ctrlid + '_param_8" size="5" value="" class="px" /></p>';
				str += '<p class="pbn">Эффект закрытия: </p><p class="pbn"><input id="' + ctrlid + '_param_7" type="radio" name="effect" checked />Нет &nbsp; <input id="' + ctrlid + '_param_5" type="radio" name="effect" />Затухание &nbsp; <input id="' + ctrlid + '_param_6" type="radio" name="effect" />Развертывание</p>';
				str += '<p class="xg2 pbn">Поддерживаюся файлы: swf, flv, jpg, gif, png <br />Размеры: Ширина 400 Высота 300 ~ 640 ~ 1024<br />пример: http://server/flash.swf</p>';
				break;
			case 'pasteword':
				stitle = 'Вставить из редактора Word';
				str = '<p class="px" style="height:300px"><iframe id="' + ctrlid + '_param_1" frameborder="0" style="width:100%;height:100%" onload="this.contentWindow.document.body.style.width=\'550px\';this.contentWindow.document.body.contentEditable=true;this.contentWindow.document.body.focus();this.onload=null"></iframe></p><p class="xg2 pbn">Используйте сочетание клавиш (Ctrl+V) для копирования текста из Word</p>';
				menuwidth = 600;
				menupos = '00';
				menutype = 'win';
				break;
			case 'index':
				stitle = 'Скрытый текст';
				str = '<p class="pbn">[index]<br />\n\
					[#<span class="xi1">номер страницы</span>]<span class="xi1">название</span> &nbsp;&nbsp;<span class="xg1">Перейти на конкретную страницу</span><br />\n\
					<span class="xi1">*</span>[#<span class="xi1">tid,pid</span>]<span class="xi1">Название</span> &nbsp;&nbsp;<span class="xg1">Перейти к конкретной странице</span><br />\n\
					[/index]<br />\n\
					<br />\n\
					<span class="xi1">Номер страницы</span> &nbsp;&nbsp;<span class="xg1">Начинать со страницы [page] </span><br />\n\
					<span class="xi1">tid,pid</span> &nbsp;&nbsp;<span class="xg1">TID и PID сообщения</span><br />\n\
					<span class="xi1">*</span> &nbsp;&nbsp;<span class="xg1">Добавить отступ первой строки</span></p>';
				break;
			default:
				...
				if(!str) {
					...
					for(var i = 1; i <= params; i++) {
						if(i != params || !haveSel) {
							str += (promptlang[i - 1] ? promptlang[i - 1] : 'Пожалуйста, введите ' + i + ' параметры:') + '<br /><input type="text" id="' + ctrlid + '_param_' + i + '" style="width: 98%" value="" class="px" />' + (i < params ? '<br />' : '');
						}
					}
				}
				break;
		...
		if(menupos == '00') {
			...
			s = '<table width="100%" cellpadding="0" cellspacing="0" class="fwin"><tr><td class="t_l"></td><td class="t_c"></td><td class="t_r"></td></tr><tr><td class="m_l">&nbsp;&nbsp;</td><td class="m_c">'
				+ '<h3 class="flb"><em>' + stitle + '</em><span><a onclick="hideMenu(\'\', \'win\');return false;" class="flbc" href="javascript:;">Увеличить</a></span></h3><div class="c">' + str + '</div>'
				+ '<p class="o pns"><button type="submit" id="' + ctrlid + '_submit" class="pn pnc"><strong>Отправить</strong></button></p>'
				+ '</td><td class="m_r"></td></tr><tr><td class="b_l"></td><td class="b_c"></td><td class="b_r"></td></tr></table>';
		} else {
			s = '<div class="p_opt cl"><span class="y" style="margin:-10px -10px 0 0"><a onclick="hideMenu();return false;" class="flbc" href="javascript:;">Увеличить</a></span><div>' + str + '</div><div class="pns mtn"><button type="submit" id="' + ctrlid + '_submit" class="pn pnc"><strong>Отправить</strong></button></div></div>';
		}
		menu.innerHTML = s;
