...
function insertAttach(id) {
	...
	if(extensions != '' && (re.exec(extensions) == null || ext == '')) {
		alert('Не поддерживается загрузка таких расширений для изображений');
		return;
	}
	...
	if(typeof no_insert=='undefined') {
		localfile += '&nbsp;<a href="javascript:;" class="xi2" title="Нажмите, чтобы вставить содержимое на место курсора" onclick="insertAttachimgTag(' + id + ');return false;">[Вставить]</a>';
	}
	inhtml += '<td><strong>' + localfile +'</strong>';
	inhtml += '</td><td class="d">Описание изображения<br/><textarea name="pic_title" cols="40" rows="2" class="pt"></textarea>';
	inhtml += '</td><td class="o"><span id="showmsg' + id + '"><a href="javascript:;" onclick="delAttach(' + id + ');return false;" class="xi2">[Удал.]</a></span>';
	inhtml += '</td></tr></table>';
...
function upload() {
	...
	if(nowUid>0) {
		...
		if(uploadStat==1) {
			upobj.innerHTML = 'Успешно загружено!';
			successState = true;
			var InputNode;
			try {
			...
			upobj.innerHTML = 'Сбой при загрузке '+uploadStat;
		}
	}

	if($('showmsg'+nid) != null) {
		$('showmsg'+nid).innerHTML = 'Загрузка, подождите пожалуйста(<a href="javascript:;" onclick="forms[nowUid].submit();">Повторить</a>)';
