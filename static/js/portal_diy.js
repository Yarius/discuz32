...
drag.extend({
	'getBlocksTimer' : '',
	'blocks' : [],
	'blockDefaultClass' : [{'key':'Выбор стиля','value':''},{'key':'Без границ и полей','value':'cl_block_bm'},{'key':'Стиль 1','value':'xbs_1'},{'key':'Стиль 2','value':'xbs xbs_2'},{'key':'Стиль 3','value':'xbs xbs_3'},{'key':'Стиль 4','value':'xbs xbs_4'},{'key':'Стиль 5','value':'xbs xbs_5'},{'key':'Стиль 6','value':'xbs xbs_6'},{'key':'Стиль 7','value':'xbs xbs_7'}],
	'frameDefaultClass' : [{'key':'Выбор стиля','value':''},{'key':'Без границ и полей','value':'cl_frame_bm'},{'key':'Без полей','value':'xfs xfs_nbd'},{'key':'Стиль 1','value':'xfs xfs_1'},{'key':'Стиль 2','value':'xfs xfs_2'},{'key':'Стиль 3','value':'xfs xfs_3'},{'key':'Стиль 4','value':'xfs xfs_4'},{'key':'Стиль 5','value':'xfs xfs_5'}],
	setDefalutMenu : function () {
		this.addMenu('default','Название','drag.openTitleEdit(event)');
		this.addMenu('default','Стиль','drag.openStyleEdit(event)');
		this.addMenu('default', 'Удалить', 'drag.removeBlock(event)');
		this.addMenu('block', 'Свойства', 'drag.openBlockEdit(event)');
		this.addMenu('block', 'Данные', 'drag.openBlockEdit(event,"data")');
		this.addMenu('block', 'Обновить', 'drag.blockForceUpdate(event)');
		this.addMenu('frame', 'Экспорт', 'drag.frameExport(event)');
		this.addMenu('tab', 'Экспорт', 'drag.frameExport(event)');
	},
	...
	openStyleEdit : function (e) {
		...
		var repeatarr = [{'key':'Заполнить','value':'repeat'},{'key':'Не заполнять','value':'no-repeat'},{'key':'Горизонтально','value':'repeat-x'},{'key':'Вертикально','value':'repeat-y'}];
		var stylearr = [{'key':'Без линии','value':'none'},{'key':'Сплошная линия','value':'solid'},{'key':'Точечная линия','value':'dotted'},{'key':'Пунктирная линия','value':'dashed'}];
		var table = '<table class="tfm">';
		table += '<tr><th>Шрифт</th><td><input type="text" id="fontsize" class="px p_fre vm" value="'+fontsize+'" size="2" />px <input type="text" id="fontcolor" class="px p_fre vm" value="'+fontcolor+'" size="2" />';
		table += getColorPalette(id+'_fontPalette', 'fontcolor' ,fontcolor)+'</td></tr>';
		table += '<tr><th>Ссылка</th><td><input type="text" id="linkfontsize" class="px p_fre vm" value="'+linkfontsize+'" size="2" />px <input type="text" id="linkcolor" class="px p_fre vm" value="'+linkcolor+'" size="2" />';
		...
		table += '<tr><th>Граница</th><td><ul id="borderul" class="'+ulclass+'">';
		table += '<li><label>Сверху</label><select class="ps vm" id="bdtwidth" ><option value="">Размер</option>'+this.getOption(widtharr,bdtwidth)+'</select>';
		table += ' <select class="ps vm" id="bdtstyle" ><option value="">Стиль</option>'+this.getOption(stylearr,bdtstyle)+'</select>';
		table += ' Цвет <input type="text" id="bdtcolor" class="px p_fre vm" value="'+bdtcolor+'" size="7" />';
		table += getColorPalette(id+'_bdtPalette', 'bdtcolor' ,bdtcolor)+'</li>';

		table += '<li class="bordera mtn"><label>Справа</label><select class="ps vm" id="bdrwidth" ><option value="">Размер</option>'+this.getOption(widtharr,bdrwidth)+'</select>';
		table += ' <select class="ps vm" id="bdrstyle" ><option value="">Стиль</option>'+this.getOption(stylearr,bdrstyle)+'</select>';
		table += ' Цвет <input type="text" id="bdrcolor" class="px p_fre vm" value="'+bdrcolor+'" size="7" />';
		table += getColorPalette(id+'_bdrPalette', 'bdrcolor' ,bdrcolor)+'</li>';

		table += '<li class="bordera mtn"><label>Снизу</label><select class="ps vm" id="bdbwidth" ><option value="">Размер</option>'+this.getOption(widtharr,bdbwidth)+'</select>';
		table += ' <select class="ps vm" id="bdbstyle" ><option value="">Стиль</option>'+this.getOption(stylearr,bdbstyle)+'</select>';
		table += ' Цвет <input type="text" id="bdbcolor" class="px p_fre vm" value="'+bdbcolor+'" size="7" />';
		table += getColorPalette(id+'_bdbPalette', 'bdbcolor' ,bdbcolor)+'</li>';

		table += '<li class="bordera mtn"><label>Слева</label><select class="ps vm" id="bdlwidth" ><option value="">Размер</option>'+this.getOption(widtharr,bdlwidth)+'</select>';
		table += ' <select class="ps vm" id="bdlstyle" ><option value="">Стиль</option>'+this.getOption(stylearr,bdlstyle)+'</select>';
		table += ' Цвет <input type="text" id="bdlcolor" class="px p_fre vm" value="'+bdlcolor+'" size="7" />';
		table += getColorPalette(id+'_bdlPalette', 'bdlcolor' ,bdlcolor)+'</li>';
		table += '</ul><p class="ptm"><label><input id="borderop" type="checkbox" value="1" class="pc"'+opchecked+' onclick="$(\'borderul\').className = $(\'borderul\').className == \'borderul\' ? \'borderula\' : \'borderul\'">Дополнительно</label></p></td></tr>';
		...
		table += '<tr><th>Поля</th><td><div id="margindiv" class="'+ulclass+'"><span><label>Сверху</label> <input type="text" id="margint" class="px p_fre vm" value="'+margint+'" size="1"/>px </span>';
		table += '<span class="bordera"><label>Справа</label> <input type="text" id="marginr" class="px p_fre vm" value="'+marginr+'" size="1" />px </span>';
		table += '<span class="bordera"><label>Снизу</label> <input type="text" id="marginb" class="px p_fre vm" value="'+marginb+'" size="1" />px </span>';
		table += '<span class="bordera"><label>Слева</label> <input type="text" id="marginl" class="px p_fre vm" value="'+marginl+'" size="1" />px</span>';
		table += '</div><p class="ptm"><label><input id="marginop" type="checkbox" value="1" class="pc"'+opchecked+' onclick="$(\'margindiv\').className = $(\'margindiv\').className == \'borderul\' ? \'borderula\' : \'borderul\'">Дополнительно</label></p></td></tr>';
		...
		if (objType == 1) {
			...
			table += '<tr><th>Поля</th><td><div id="cmargindiv" class="'+ulclass+'"><span><label>Сверху</label> <input class="px p_fre" id="cmargint" value="'+cmargint+'" size="1" />px </span>';
			table += '<span class="bordera"><label>Справа</label> <input class="px p_fre" id="cmarginr" value="'+cmarginr+'" size="1" />px </span>';
			table += '<span class="bordera"><label>Снизу</label> <input class="px p_fre" id="cmarginb" value="'+cmarginb+'" size="1" />px </span>';
			table += '<span class="bordera"><label>Слева</label> <input class="px p_fre" id="cmarginl" value="'+cmarginl+'" size="1" />px </span>';
			table += '</div><p class="ptm"><label><input id="cmarginop" type="checkbox" value="1" class="pc"'+opchecked+' onclick="$(\'cmargindiv\').className = $(\'cmargindiv\').className == \'borderul\' ? \'borderula\' : \'borderul\'"> Дополнительно</label></p></td></tr>';
		}
		table += '<tr><th>Цвет фона</th><td><input type="text" id="bgcolor" class="px p_fre vm" value="'+bgcolor+'" size="4" />';
		table += getColorPalette(id+'_bgcPalette', 'bgcolor' ,bgcolor)+'</td></tr>';
		table += '<tr><th>Фоновое изображение</th><td><input type="text" id="bgimage" class="px p_fre vm" value="'+bgimage+'" size="25" /> <select class="ps vm" id="bgrepeat" >'+this.getOption(repeatarr,bgrepeat)+'</select></td></tr>';
		var classarr = objType == 1 ? this.blockDefaultClass : this.frameDefaultClass;
		table += '<tr><th>Укажите class</th><td><input type="text" id="diyClassName" class="px p_fre" value="'+diyClassName+'" size="8" /> <select class="ps vm" id="bgrepeat" onchange="$(\'diyClassName\').value=this.value;" >'+this.getOption(classarr, diyClassName)+'</select></td></tr>';
		table += '</table>';

		var wname = objType ? 'Модуль' : 'Рамка';
		html = '<div class="c diywin" style="width:450px;position:relative;">'+table+'</div>';
		var h = '<h3 class="flb"><em>Изменить '+wname+' стиль </em><span><a href="javascript:;" class="flbc" onclick="drag.closeStyleEdit(\''+id+'\');return false;" title="Закрыть">\n\
			Закрыть</a></span></h3>';
		var f = '<p class="o pns"><button onclick="drag.saveStyle(\''+id+'\');drag.closeStyleEdit(\''+id+'\');" class="pn pnc" value="true">\n\
			<strong>ОК</strong></button><button onclick="drag.closeStyleEdit(\''+id+'\')" class="pn" value="true"><strong>Отменить</strong></button></p>';
		this.removeMenu(e);
	...
	openTitleEdit : function (e) {
		...
		var titlename = obj instanceof Block ? 'Модуль' : 'Рамка';
		var repeatarr = [{'key':'Заливка','value':'repeat'},{'key':'Без повтора','value':'no-repeat'},{'key':'Горизонтально','value':'repeat-x'},{'key':'Вертикально','value':'repeat-y'}];
		...
		common += '<tr><th>Фоновое изображение: </th><td><input type="text" id="titleBgImage" class="px p_fre" value="'+bgimage+'" /> <select class="ps vm" id="titleBgRepeat" >'+this.getOption(repeatarr,bgrepeat)+'</select></td></tr>';
		common += '<tr><th>Цвет фона: </th><td><input type="text" id="titleBgColor" class="px p_fre" value="'+bgcolor+'" size="7" />';
		common += getColorPalette(fid+'bgPalette_0', 'titleBgColor' ,bgcolor)+'</td></tr>';
		if (obj instanceof Tab) {
			var switchArr = [{'key':'Нажмите','value':'click'},{'key':'Переместить','value':'mouseover'}];
			var switchType = obj.titles['switchType'] ? obj.titles['switchType'][0] : 'click';
			common += '<tr><th>Тип переключения:</th><td><select class="ps" id="switchType" >'+this.getOption(switchArr,switchType)+'</select></td></tr>';
		}
		common += '</table><hr class="l">';
		var li = '';
		li += '<div id="titleInput_0"><table class="tfm"><tr><th>'+titlename+'Название:</th><td><input type="text" id="titleText_0" class="px p_fre" value="`title`" /></td></tr>';
		li += '<tr><th>Ссылки:</th><td><input type="text" id="titleLink_0" class="px p_fre" value="`link`" /></td></tr>';
		li += '<tr><th>Фото:</th><td><input type="text" id="titleSrc_0" class="px p_fre" value="`src`" /></td></tr>';
		li += '<tr><th>Положение:</th><td><select id="titleFloat_0" class="ps vm"><option value="" `left`>Главная слева</option><option value="right" `right`>Главная справа</option></select>';
		li += '&nbsp;&nbsp;Смещение: <input type="text" id="titleMargin_0" class="px p_fre vm" value="`margin`" size="2" />px</td></tr>';
		li += '<tr><th>Шрифт:</th><td><select class="ps vm" id="titleSize_0" ><option value="">Размер</option>`size`</select>';
		li += '&nbsp;&nbsp;Цвет: <input type="text" id="titleColor_0" class="px p_fre vm" value="`color`" size="4" />';
		...
		var c = len + 1;
			html = '<div class="c diywin" style="width:450px;height:400px; overflow:auto;"><table cellspacing="0" cellpadding="0" class="tfm pns"><tr><th></th><td><button type="button" id="addTitleInput" class="pn" onclick="drag.addTitleInput('+c+');"><em>Добавить название</em></button></td></tr></table><div id="titleEdit">'+html+common+'</div></div>';
		var h = '<h3 class="flb"><em>Изменить '+titlename+' название</em><span><a href="javascript:;" class="flbc" onclick="drag.closeTitleEdit(\''+fid+'\');return false;" title="Закрыть">\n\
			Закрыть</a></span></h3>';
		var f = '<p class="o pns"><button onclick="drag.saveTitleEdit(\''+fid+'\');drag.closeTitleEdit(\''+fid+'\');" class="pn pnc" value="true">\n\
			<strong>ОК</strong></button><button onclick="drag.closeTitleEdit(\''+fid+'\')" class="pn" value="true"><strong>Отменить</strong></button></p>';
		this.removeMenu(e);
	...
	removeBlock : function (e, flag) {
		...
		var obj = this.getObjByName(id);
		if (!flag) {
			if (!confirm('Вы уверены, что хотите удалить? Данное действие невозможно будет восстановить!')) return false;
		}
		if (obj instanceof Block) {
			this.delBlock(id);
		} else if (obj instanceof Frame) {
	...
	blockForceUpdate : function (e,all) {
		...
		bcontent.innerHTML = '<center>Идет загрузка...</center>';
		var x = new Ajax();
		x.get('portal.php?mod=portalcp&ac=block&op=getblock&forceupdate=1&inajax=1&bid='+bid+'&tpl='+document.diyform.template.value, function(s) {
			if(s.indexOf('errorhandle_') != -1) {
				...
				runslideshow();
				showDialog('Извините, у вас нет разрешения на добавление или изменение модуля', 'alert');
				doane();
			} else {
	...
	frameExport : function (e) {
		var flag = true;
		if (drag.isChange) {
			flag = confirm('Вы внесли изменения. Советуем сохранить, а затем сделать экспорт полученных результатов!');
		}
		if (flag) {
			if ( typeof e == 'object') {
				e = Util.event(e);
	...
	endBlockForceUpdateBatch : function () {
		if($('allupdate')) {
			$('allupdate').innerHTML = 'Данные успешно обновлены!';
	...
	getBlocks : function () {
		...
		if (this.blocks.length > 0) {
			...
			if($('allupdate')) {
				$('allupdate').innerHTML = 'Всего: <span style="color:blue">'+this.blocksLen+'</span> Обновлено: <span style="color:red">'+cur+'</span> Завершено: <span style="color:red">'+(parseInt(cur / this.blocksLen * 100)) + '%</span>';
	...
	blockForceUpdateBatch : function (blocks) {
		...
		showDialog('<div id="allupdate" style="width:350px;line-height:28px;">Начало обновления...</div>','confirm','Обновление данных модулей', '', true, 'drag.endBlockForceUpdateBatch()');
		var wait = function() {
	...
	clearAll : function () {
		if (confirm('Вы уверены, что хотите очистить данные на странице? После очистки удалятся все данные и страница будет пустой!')) {
	...
	createObj : function (e,objType,contentType) {
		if (objType == 'block' && !this.checkHasFrame()) {alert("Совет: рамки не найдены, пожалуйста, добавьте рамку!");spaceDiy.getdiy('frame');return false;}
		e = Util.event(e);
	...
	getFrameHtml : function (type) {
		...
		str += '<div id="'+id+'_title" class="'+this.titleClass+' '+this.frameTitleClass+'"><span class="'+this.titleTextClass+'">'+type+'Рамка</span></div>';
		var cols = type.split('-');
		var clsl='',clsc='',clsr='';
		clsl = ' frame-'+type+'-l';
		clsc = ' frame-'+type+'-c';
	...
	getTabHtml : function () {
		...
		var str = '<div id="'+id+'" class="'+className+'">';
		str += '<div id="'+id+'_title" class="'+titleClassName+'"><span class="'+this.titleTextClass+'">Вкладка</span></div>';
		...
		if(this.sampleMode) {
			...
			if (!this.isChange) {
				window.onbeforeunload = function() {
					return 'Ваши данные были изменены, если Вы покинете страницу изменения не будут сохранены!';
				};
			}
			this.isChange = true;
	...
	goonDIY : function () {
		if ($('prefile').value == '1') {
			showDialog('<div style="line-height:28px;">Нажмите кнопку Продолжить для открытия временных данных ВИЗО, </div>','confirm','Нажмите кнопку Удалить для удаления всех временных данных. ', function(){location.replace(location.href+'&preview=yes');}, true, 'spaceDiy.cancelDIY()', '', 'Продолжить', 'Удалить');
...
spaceDiy.extend({
		...
		if (!optype) {
			if (['portal/portal_topic_content', 'portal/list', 'portal/view'].indexOf(tplpre[0]) == -1) {
				if (document.diyform.template.value.indexOf(':') > -1 && !document.selectsave) {
					...
					showDialog('<form name="selectsave" action="" method="get"><label><input type="radio" value="0" name="savemod"'+schecked+' />Применить ко всем страницам</label>\n\
					<label><input type="radio" value="1" name="savemod"'+dchecked+' />Применить только к этой странице</label></form>','notice', '', spaceDiy.save);
					return false;
				}
	...
	cancel : function () {
		...
		if (drag.isClearClose) {
			showDialog('<div style="line-height:28px;">Что сделать с временными данными?<br />Нажмите кнопку ОК, чтобы сохранить временные данные. Нажмите кнопку Отмена, чтобы удалить временные данные.</div>','confirm','Сохранение временных данных', function(){location.href = spaceDiy.cancelDiyUrl();}, true, function(){window.onunload=function(){spaceDiy.cancelDIY()};location.href = spaceDiy.cancelDiyUrl();});
		} else {
	...
	recover : function() {
		if (confirm('Вы уверены, что хотите восстановить предыдущую версию для сохранения результатов?')) {
			drag.clearClose();
