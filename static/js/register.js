...
var lastusername = '', lastpassword = '', lastemail = '', lastinvitecode = '', stmp = new Array(), modifypwd = false, profileTips = 'Оставьте пустым, если не хотите изменить пароль';
...
function checkPwdComplexity(firstObj, secondObj, modify) {
	...
	firstObj.onblur = function () {
		if(firstObj.value == '') {
			var pwmsg = !modifypwd ? 'Введите пароль' : profileTips;
			if(pwlength > 0) {
				pwmsg += ', Минимальная длина '+pwlength+' символов';
			}
	...
	firstObj.onkeyup = function () {
		if(pwlength == 0 || $(firstObj.id).value.length >= pwlength) {
			var passlevels = new Array('','Слабый','Средний','Сильный');
			var passlevel = checkstrongpw(firstObj.id);
			errormessage(firstObj.id, '<span class="passlevel passlevel'+passlevel+'">Надежность пароля:'+passlevels[passlevel]+'</span>');
		}
	};
	secondObj.onblur = function () {
		if(secondObj.value == '') {
			errormessage(secondObj.id, !modifypwd ? 'Повторите пароль' : profileTips);
		}
		checkpassword(firstObj.id, secondObj.id);
	};
}
...
function addMailEvent(mailObj) {
	...
	mailObj.onblur = function () {
		if(mailObj.value == '') {
			errormessage(mailObj.id, 'Введите E-mail');
		}
...
function showbirthday(){
	...
	el.length=0;
	el.options.add(new Option('Дата', ''));
	for(var i=0;i<28;i++){
...
var emailMenuST = null, emailMenui = 0, emaildomains = ['mail.ru', 'gmail.com', 'yandex.ru', 'list.ru', 'bk.ru', 'rambler.ru', 'hotmail.com'];
...
function checkusername(id) {
	...
	if(username.match(/<|"/ig)) {
		errormessage(id, 'Имя пользователя содержит недопустимые символы!');
		return;
	}
	var unlen = username.replace(/[^\x00-\xff]/g, "**").length;
	if(unlen < 3 || unlen > 25) {
		errormessage(id, unlen < 3 ? 'Не менее трех символов ':' Не более 25 символов');
		return;
	}
	var x = new Ajax();
	$('tip_' + id).parentNode.className = $('tip_' + id).parentNode.className.replace(/ p_right/, '');
...
function checkpassword(id1, id2) {
	...
	if(pwlength > 0) {
		if($(id1).value.length < pwlength) {
			errormessage(id1, 'Пароль слишком короткий, не менее '+pwlength+' символов');
			return;
		}
	}
	if(strongpw) {
		...
		for(var i in strongpw) {
			if(strongpw[i] === 1 && !$(id1).value.match(/\d+/g)) {
				strongpw_error = true;
				strongpw_str[j] = 'Цифры';
				j++;
			}
			if(strongpw[i] === 2 && !$(id1).value.match(/[a-z]+/g)) {
				strongpw_error = true;
				strongpw_str[j] = 'Строчные буквы';
				j++;
			}
			if(strongpw[i] === 3 && !$(id1).value.match(/[A-Z]+/g)) {
				strongpw_error = true;
				strongpw_str[j] = 'Заглавные буквы';
				j++;
			}
			if(strongpw[i] === 4 && !$(id1).value.match(/[^A-Za-z0-9]+/g)) {
				strongpw_error = true;
				strongpw_str[j] = 'Специальные символы';
				j++;
			}
		}
		if(strongpw_error) {
			errormessage(id1, 'Слабый пароль, пароль должен содержать '+strongpw_str.join('，'));
			return;
		}
	}
	errormessage(id2);
	if($(id1).value != $(id2).value) {
		errormessage(id2, 'Пароль и его подтверждение не совпадают');
	} else {
...
function checkemail(id) {
	...
	if(email.match(/<|"/ig)) {
		errormessage(id, 'E-mail содержит недопустимые символы');
		return;
	}
	var x = new Ajax();
	$('tip_' + id).parentNode.className = $('tip_' + id).parentNode.className.replace(/ p_right/, '');
...
function checkinvite() {
	...
	if(invitecode.match(/<|"/ig)) {
		errormessage('invitecode', 'Приглашение содержит недопустимые символы!');
		return;
	}
	var x = new Ajax();
