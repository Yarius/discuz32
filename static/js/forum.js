...
function removeindexheats() {
	return confirm('Вы, действительно, хотите удалить тему из важных тем?');
}
...
function fastpostvalidate(theform, noajaxpost) {
	...
	if(theform.message.value == '' || theform.subject.value == '') {
		s = 'Введите название темы или текст сообщения';
		theform.message.focus();
	} else if(mb_strlen(theform.subject.value) > 180) {
		s = 'Название превышает 180 символов';
		theform.subject.focus();
	}
	if(!disablepostctrl && ((postminchars != 0 && mb_strlen(theform.message.value) < postminchars) || (postmaxchars != 0 && mb_strlen(theform.message.value) > postmaxchars))) {
		s = 'Длина Вашего сообщения не соответствуют требованиям!\n\nТекущая длина: ' + mb_strlen(theform.message.value) + ' ' + 'символов\nЛимит системы от: ' + postminchars + ' до ' + postmaxchars + ' символов';
	}
	if(s) {
		showError(s);
		doane();
...
function loadData(quiet, formobj) {
	...
	if(in_array((data = trim(data)), ['', 'null', 'false', null, false])) {
		if(!quiet) {
			showDialog('Нет данных для восстановления!', 'info');
		}
		return;
	}

	if(!quiet && !confirm('Это действие перезапишет содержимое текущего сообщения, Вы уверены, что хотите восстановить данные?')) {
		return;
	}
...
function checkForumnew(fid, lasttime) {
	...
	x.get('forum.php?mod=ajax&action=forumchecknew&fid=' + fid + '&time=' + lasttime + '&inajax=yes', function(s){
		if(s > 0) {
			...
			var checknew = {'tid':'', 'thread':{'common':{'className':'', 'val':'<a href="javascript:void(0);" onclick="ajaxget(\'forum.php?mod=ajax&action=forumchecknew&fid=' + fid+ '&time='+lasttime+'&uncheck=1&inajax=yes\', \'forumnew\');">В теме появилось новое сообщение.Нажмите для просмотра.', 'colspan': colspan }}};
