...
var ATTACHORIMAGE = '0';
var STATUSMSG = {
	'-1' : 'Внешняя ошибка сервера!',
	'0' : 'Файл успешно загружен!',
	'1' : 'Данное расширение не поддерживается!',
	'2' : 'Превышен размер для вложений!',
	'3' : 'Превышен размер для вложений для Вашей группы!',
	'4' : 'Данное расширение не поддерживается!',
	'5' : 'Запрещенный тип файла или превышен лимит размера',
	'6' : 'Извините, но для Вас, на сегодня, лимит по количеству файлов исчерпан!',
	'7' : 'Пожалуйста, выберите файл изображения (' + imgexts + ')',
	'8' : 'Файл не может быть сохранен!',
	'9' : 'Файл содержит недопустимые данные!',
	'10' : 'Недопустимая операция!',
	'11' : 'Извините, но для Вас, на сегодня, лимит по размеру файлов исчерпан!'
};
...
function checklength(theform) {
	...
	showDialog('Текущая длина: ' + mb_strlen(message) + ' символов,' + (postmaxchars != 0 ? 'Системные ограничения: ' + postminchars + ' символов ' + postmaxchars + ' байт.' : ''), 'notice', 'Проверить длину');
}
...
function validate(theform) {
	...
	if(($('postsubmit').name != 'replysubmit' && !($('postsubmit').name == 'editsubmit' && !isfirstpost) && theform.subject.value == "") || !sortid && !special && trim(message) == "") {
		showError('Введите название или текст');
		return false;
	} else if(mb_strlen(theform.subject.value) > 180) {
		showError('Название превышает 180 символов');
		return false;
	}
	...
	if(!disablepostctrl && !sortid && !special && ((postminchars != 0 && mb_strlen(message) < postminchars) || (postmaxchars != 0 && mb_strlen(message) > postmaxchars))) {
		showError('Длина сообщения не соответствует требованиям\n\nТекущая длина: ' + mb_strlen(message) + ' символов\nЛимит системы: ' + postminchars + ' -- ' + postmaxchars + ' символов');
		return false;
	}
	if(UPLOADSTATUS == 0) {
		if(!confirm('Имеются загруженные вложения! Вы не хотите использовать эти вложения?')) {
			return false;
		}
	} else if(UPLOADSTATUS == 1) {
		showDialog('Загрузка вложений, пожалуйста подождите...', 'notice');
		AUTOPOST = 1;
		return false;
	}
	if(isfirstpost && $('adddynamic') != null && $('adddynamic').checked && $('postsave') != null && isNaN(parseInt($('postsave').value)) && ($('readperm') != null && $('readperm').value || $('price') != null && $('price').value)) {
		if(confirm('Потому что вы установили разрешения на чтение или продать сообщений, Вы подтверждаете, что также транслируется на аудиторию, чтобы увидеть?') == false) {
			return false;
		}
	}
		...
		if(seccodecheck || secqaacheck) {
			...
			if(secqaacheck) {
				...
				if(chkv.indexOf('loading') != -1) {
					...
					showError('Введите символы проверочного кода');
					chk = 0;
				}
			}
			if(seccodecheck) {
...
function uploadNextAttach() {
	...
	var sizelimit = '';
	if(arr[4] == 'ban') {
		sizelimit = '(Запрещенный тип вложений!)';
	} else if(arr[4] == 'perday') {
		sizelimit = '(Не может превышать ' + arr[5] + ' байт)';
	} else if(arr[4] > 0) {
		sizelimit = '(Не может превышать ' + arr[4] + ' байт)';
	}
...
function uploadAttach(curId, statusid, prefix, sizelimit) {
	...
	if(curId > 0) {
		...
		if(nextId == curId || in_array(statusid, [6, 8])) {
			...
			if(UPLOADFAILED > 0) {
				showDialog('Загрузка завершена, все вложения успешно загружены' + UPLOADCOMPLETE + ' Ошибка загрузки! ' + UPLOADFAILED + ' Файлы:' + FAILEDATTACHS);
				FAILEDATTACHS = '';
			}
			UPLOADSTATUS = 2;
			for(var i = 0; i < AID[prefix ? 1 : 0] - 1; i++) {
			...
			if(AUTOPOST) {
				...
				showDialog('Вложения успешно загружены', 'right', null, null, 0, null, null, null, null, 3);
			}
	...
	$(prefix + 'cpdel_' + nextId).innerHTML = '<img src="' + IMGDIR + '/loading.gif" alt="Загрузка..." />';
	UPLOADSTATUS = 1;
...
function insertAttach(prefix, id) {
	...
	if(extensions != '' && (re.exec(extensions) == null || ext == '')) {
		...
		showError('Нельзя загружать файлы с таким расширением');
		return;
	}
	if(prefix == 'img' && imgexts.indexOf(ext) == -1) {
		reAddAttach(prefix, id);
		showError('Выберите изображение(' + imgexts + ')');
		return;
	}

	$(prefix + 'cpdel_' + id).innerHTML = '<a href="javascript:;" class="d" onclick="reAddAttach(\'' + prefix + '\', ' + id + ')">Удалить</a>';
...
function updateattachnum(type) {
	...
	if(num) {
		if($(editorid + '_' + type)) {
			$(editorid + '_' + type).title = 'Содержит ' + num + (type == 'image' ? ' Вложение изображений' : ' Вложения');
		}
		...
		if($(editorid + '_' + type)) {
			$(editorid + '_' + type).title = type == 'image' ? 'Фотографии' : 'Файлы';
		}
		if($(editorid + '_' + type + 'n')) {
...
function updateDownImageList(msg) {
	...
	if(msg == '') {
		showError('К сожалению, нет удаленных файлов');
	} else {
		...
		switchImagebutton('imgattachlist');$('imgattach_notice').style.display = '';
		showDialog('Удаленная загрузка вложений завершена', 'right', null, null, 0, null, null, null, null, 3);
	}
}
...
function uploadWindowload() {
	...
	if(arr[0] == 'DISCUZUPLOAD' && arr[2] == 0) {
		...
		if(arr[7] == 'ban') {
			...
			sizelimit = '(Не может превышать ' + arr[8] + ' байт)';
		} else if(arr[7] > 0) {
			sizelimit = '(Не может превышать ' + arr[7] + ' байт)';
		}
...
function addpolloption() {
	if(curoptions < maxoptions) {
		...
		$('polloption_new').outerHTML = '<span>Превышен лимит для голосований!'+maxoptions+'</span>';
	}
}
...
function attachoption(type, op) {
	if(!op) {
		...
		showDialog('<div id="unusedwin" class="c altw" style="overflow:auto;height:100px;">' + $('unusedlist_' + type).innerHTML + '</div>' +
			'<p class="o pns"><span class="z xg1"><label for="unusedwinchkall"><input id="unusedwinchkall" type="checkbox" onclick="attachoption(\'' + type + '\', 3)" checked="checked" />Выбрать все</label></span>' +
			'<button onclick="attachoption(\'' + type + '\', 1);hideMenu(\'fwin_dialog\', \'dialog\')" class="pn pnc"><strong>Прикрепить</strong></button></p>', 'info', 'Неиспользованные ' + (type == 'attach' ? 'файлы' : ' изображения'));
	} else if(op == 3) {
...
function getreplycredit() {
	...
	if(real_reply_credit > userextcredit) {
		$('replycredit').innerHTML = '<b class="xi1">Превышена сумма вознаграждения за ответ('+real_reply_credit+')</b>';
	} else {
		if(have_replycredit > 0 && real_reply_credit < 0) {
			$('replycredit').innerHTML = "<font class='xi1'>Вернуться"+Math.abs(real_reply_credit)+"</font>";
		} else {
...
function deleteThread() {
	if(confirm('Вы уверены, что хотите удалить это сообщение?') != 0){
		$('delete').value = '1';
		$('postform').submit();
	}
}
