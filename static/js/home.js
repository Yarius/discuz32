		...
		theImages[i].onload = function() {
			if (this.width > size) {
				...
				if (this.parentNode.tagName.toLowerCase() != 'a') {
					...
					this.title = 'Нажмите на изображение, для открытия оригинального размера. Откроется в новом окне';
...
function ischeck(id, prefix) {
	...
	for(var i = 0; i < form.elements.length; i++) {
		...
		if(e.name.match(prefix) && e.checked) {
			if(confirm("Вы уверены, что хотите выполнить эту операцию?")) {
				return true;
			} else {
				return false;
			}
...
function insertWebImg(obj) {
	if(checkImage(obj.value)) {
		...
		alert('Неверный адрес изображения');
	}
}
...
function showFlash(host, flashvar, obj, shareid) {
	...
	var videoMp3 = true;
	if('' == flashvar) {
		alert('Адрес аудио файла отсутствует или введен некорректно');
		return false;
	}
	if('music' == host) {
	...
	if(flashAddr[host]) {
		...
		var nodetxt = document.createTextNode("Закрыть");
...
function userapp_close() {
	...
	x.get('home.php?mod=spacecp&ac=common&op=getuserapp&subop=off&inajax=1', function(s){
		...
		$('a_app_more').innerHTML = 'Развернуть';
		$('a_app_more').onclick = function() {
			userapp_open();
		};
	});
...
function myfriend_post(uid) {
	if($('friend_'+uid)) {
		$('friend_'+uid).innerHTML = '<p>Вы стали друзьями, теперь вы можете: <a href="home.php?mod=space&do=wall&uid='+uid+'" class="xi2" target="_blank">Отправить сообщение</a> , или <a href="home.php?mod=spacecp&ac=poke&op=send&uid='+uid+'&handlekey=propokehk_'+uid+'" id="a_poke_'+uid+'" class="xi2" onclick="showWindow(this.id, this.href, \'get\', 0, {\'ctrlid\':this.id,\'pos\':\'13\'});">Отправить привет</a></p>';
	}
	showCreditPrompt();
}
...
function feed_more_show(feedid) {
	...
	$(opid).innerHTML = '&laquo; Свернуть список';
	$(opid).onclick = function() {
...
function feed_more_close(feedid) {
	...
	$(opid).innerHTML = '&raquo; Все новости';
	$(opid).onclick = function() {
