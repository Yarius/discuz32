...
function showloading(display, waiting) {
	...
	var waiting = waiting ? waiting : 'Загрузка...';
...
function showDialog(msg, mode, t, func, cover, funccancel, leftmsg, confirmtxt, canceltxt, closetime, locationtime) {
	...
	if(locationtime) {
		leftmsg = locationtime + ' сек. до перехода на другую страницу';
		showDialogST = setTimeout(closefunc, locationtime * 1000);
		showconfirm = 0;
	}
	confirmtxt = confirmtxt ? confirmtxt : confirmtxtdefault;
	canceltxt = canceltxt ? canceltxt : 'Отмена';
	...
	s += t ? t : 'Сообщение';
	s += '</em><span><a href="javascript:;" id="fwin_dialog_close" class="flbc" onclick="hideMenu(\'' + menuid + '\', \'dialog\')" title="Закрыть">Закрыть</a></span></h3>';
	if(mode == 'info') {
		s += msg ? msg : '';
	} else {
...
function showWindow(k, url, mode, cache, menuv) {
	...
	var fetchContent = function() {
		...
		if(parseInt(BROWSER.ie) != 6) {
			loadingst = setTimeout(function() {showDialog('', 'info', '<img src="' + IMGDIR + '/loading.gif"> Загрузка...')}, 500);
		}
	};
...
function showError(msg) {
	...
	msg = msg.replace(p, '');
	if(msg !== '') {
		showDialog(msg, 'alert', 'Сообщение об ошибке', null, true, null, '', '', '', 3);
	}
}
...
function initSearchmenu(searchform, cloudSearchUrl) {
	...
	if (!!("placeholder" in document.createElement("input"))) {
		if(searchtxt.value == 'Поиск по содержанию') {
			searchtxt.value = '';
		}
		searchtxt.placeholder = 'Поиск по содержанию';
	} else {
		...
		searchtxt.onblur = function () {
			if(searchtxt.value == '' ) {
				searchtxt.value = 'Поиск по содержанию';
				searchtxt.className = tclass + ' xg1';
			}
		};
	}
...
function searchFocus(obj) {
	if(obj.value == 'Введите хоть что-нибудь') {
		obj.value = '';
	}
...
function noticeTitleFlash() {
	if(NOTICETITLE.flashNumber < 5 || NOTICETITLE.flashNumber > 4 && !NOTICETITLE['State']) {
		document.title = (NOTICETITLE['State'] ? '[　　　]' : '[Пришло новое уведомление!]') + NOTICETITLE['oldTitle'];
...
function addFavorite(url, title) {
	try {
		...
		try {
			...
			showDialog("Нажмите Ctrl+D для добавления сайта в избранное", 'notice');
		}
	}
}
...
function setHomepage(sURL) {
	if(BROWSER.ie){
		...
		showDialog("Только для браузера IE. Для других браузеров настройте Стартовую страницу вручную.", 'notice');
		doane();
	}
}
...
function toggleBlind(dom) {
	if(dom) {
		if(loadUserdata('is_blindman')) {
			...
			dom.title = 'Открыть вспомогательный доступ';
		} else {
			saveUserdata('is_blindman', '1');
			dom.title = 'Закрыть вспомогательный доступ';
		}
	}
}
