	...
	x.getJSON(url+'&_makehtml&r='+(+ new Date()), function(ret){
		...
		if(ret && (ret=ret['data']) && ret['status'] == 'html_ok') {
			if(obj) {
				...
				obj.innerHTML = '<a href="'+ret['path']+'" target="_blank">'+title+'Успешно создан</a>';
			}
			if(ret['nexturl']) {
				if(obj) {
					obj.style.color = 'green';
					obj.innerHTML = 'Создать'+title+(Math.round((ret['current']/ret['count'])*100))+'%';
				}
			...
			if(obj) {
				obj.style.color = 'red';
				obj.innerHTML = title+'Ошибка при создании';
			}
		}
	});
...
function make_html_batch(url, ids, callback, dom, single) {
	...
	if(this.single) {
		...
		progress_bar(cent);
		child.innerHTML = 'Начать создание'+this.dom.getAttribute('mktitle');
		this.dom.innerHTML = '';
		this.dom.appendChild(child);
		this.make(id, child);
		this.child = child;
		var child2 = document.createElement('div');
		child2.innerHTML = '<a href="javascript:void(0);" id="mk_goon">Если Ваш браузер не отвечает, нажмите кнопку Продолжить...</a>';
		...
		x.getJSON(this.url+id+'&_makehtml&r='+(+ new Date()), function(ret){
			if(ret && (data=ret['data']) && data['status'] == 'html_ok') {
				...
				if(!makehtml_error) {
					...
					makehtml_error.innerHTML = 'Сообщения об ошибке';
			...
			if(obj.single) {
				...
				progress_bar(cent);
				var str = 'Всего нужно создать'+obj.count+'шт.'+obj.dom.getAttribute('mktitle')+'Файл успешно создан'+obj.makedcount+'个，';
				if(cent != 100) {
					child.innerHTML = str+'Создание первого'+current+'месяца был завершен'+cent+'%';
				} else {
					child.style.color = 'blue';
					child.innerHTML = str+obj.dom.getAttribute('mktitle')+'Создание завершено';
				}
				if(id) {
					obj.make(id, child);
